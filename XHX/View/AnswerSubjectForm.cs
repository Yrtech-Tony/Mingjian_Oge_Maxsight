    ...
    public partial class AnswerSubjectForm : Form
    {
        ...
        private void btnPictureUpload_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (uploaddto.FilePath.Contains(':') && uploaddto.FilePath.Contains('\\'))
            {
                ...
                //    image = new byte[0];
                //}
                aliyun.PutObject("yrtech", "Oge" + @"/" + CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text + @"/" + SubjectCode_Golbal + @"/" + uploaddto.FileName + ".jpg", filePath);
        ...
        private void grcLoss_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (bdhi.InRow)
            {
                ...
                for (int i = 0; i < filenameList.Length; i++)
                {
                    ...
                    //    image = new byte[0];
                    //}
                    aliyun.PutObject("yrtech", "Oge" + @"/" + CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text + @"/" + SubjectCode_Golbal + @"/" + filenameList[i] + ".jpg", filePath);
