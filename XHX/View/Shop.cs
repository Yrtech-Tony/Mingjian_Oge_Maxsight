...
namespace XHX.View
{
    public partial class Shop : BaseForm
    {
        ...
        {
            Workbook workbook = msExcelUtil.OpenExcelByMSExcel(btnModule.Text);
            Worksheet worksheet_FengMian = workbook.Worksheets["ShopArea"] as Worksheet;
            for (int i = 2; i < 400; i++)
            {
                string shopCode = msExcelUtil.GetCellValue(worksheet_FengMian, "A", i).Trim();
                if (!string.IsNullOrEmpty(shopCode))
                {
                    string areaCode_sale = msExcelUtil.GetCellValue(worksheet_FengMian, "B", i);
                    string areaCode_after = msExcelUtil.GetCellValue(worksheet_FengMian, "C", i);
                    webService.SaveShopArea(areaCode_sale, areaCode_after, shopCode);
                }
